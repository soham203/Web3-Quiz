...
const QuizComponent = () => {
  const { topic, difficulty } = useParams(); // Get the topic and difficulty from URL params
  ...
  useEffect(() => {
    const loadQuestions = async () => {
      try {
        const data = await import(`../data/${topic.charAt(0).toUpperCase() + topic.slice(1)}Data.json`);
        
        if (data[topic] && data[topic][difficulty]) {
          const filteredQuestions = data[topic][difficulty];
  ...
  const currentQuestions = questions.slice(indexOfFirstQuestion, indexOfLastQuestion); // Get only 5 questions per page

  // Pagination buttons
  const totalPages = Math.ceil(questions.length / questionsPerPage);
  const handlePageChange = (pageNumber) => setCurrentPage(pageNumber); // Change current page
